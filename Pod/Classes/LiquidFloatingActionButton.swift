...
public class LiquidFloatingActionButton : UIView {
    ...
    fileprivate var plusRotation: CGFloat = 0
    ...
    var TempData=[LiquidFloatingCell]()
    private func cellArray() -> [LiquidFloatingCell] {
        ...
        //print("dataSourcedataSource \(dataSource)")
        print("resultresult \(result.count)")
        return result
    }
    ...
    public func open() {
        ...
        self.plusRotation = CGFloat(Double.pi * 0.25) // 45 degree
    ...
    private func drawCircle() {
        ...
        if touching && responsible {
            self.circleLayer.backgroundColor = UIColor.white.cgColor
        } else {
            self.circleLayer.backgroundColor = self.color.cgColor
        }
    }
    
    fileprivate func drawPlus(_ rotation: CGFloat) {
        
        let myLayer = CALayer()
        let myImage = UIImage(named: "icon_share_new.png")?.cgImage
        
        myLayer.frame = CGRect(x: 0, y: 0, width: circleLayer.frame.width , height: circleLayer.frame.height)
        
        myLayer.contents = myImage
        
        plusLayer.frame = CGRect(origin: CGPoint.zero,size: self.frame.size)
        plusLayer.addSublayer(myLayer)
    ...
    public func didTappedCell(target: LiquidFloatingCell) {
        print("targettargettarget \(target)")
        if let _ = dataSource {
            let cells = cellArray()
...
class CircleLiquidBaseView : ActionBarBaseView {
    ...
    public var openingCells: [LiquidFloatingCell] = []
